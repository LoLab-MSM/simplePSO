...
solver = BNGSSASimulator(model,tspan,verbose=True)
...
def likelihood(position):
    ...
    solver.run(param_values,verbose=True)
...
pso.set_bounds(2)
pso.set_speed(-.25,.25)
values = pso.run(50,100)
#print values
#display(pso.best)
run = 'run_18'
fit_filename = '%s_EARM_2_1_fit_params.txt'%run
import pysb
param_values[rate_mask] = 10**pso.best
pysb.util.write_params(model, param_values, fit_filename)
...
def plot_mean_min_max(name, title=None):
    ...
    solver.run(param_values)
    plt.plot(solver.tspan,solver.yobs[name],'ko-',label='ODE')
    plt.legend(loc=0)
    plt.xlabel('Time')
    plt.ylabel('Population of %s' %name)
    plt.savefig('%s_%s.png'%(run,name))
    plt.savefig('%s_%s.pdf'%(run,name))

tout, trajectories = run_stochkit(model, tspan,param_values, n_runs=100, seed=None, verbose=True)
#quit()
