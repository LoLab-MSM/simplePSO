...
SPECIES_PARAMETERS = 14

# service
colors = ["red", "blue", "green", "gold"]


class PE_functor(object):
     ...
     def fitness(self, parameters):
          ...
          if self._MOO: 
               ...
               INITIAL_CONDITIONS = [23,233,2326]
               res=[]

               for EXP_CONDITION in xrange(3):
                    SPECIFIC_CONDITION = append([0], arguments['time_series'][EXP_CONDITION])
                    sim = BngSimulator(model, tspan=arguments['time_points'])          
                    model.parameters['TNF_0'].value=INITIAL_CONDITIONS[EXP_CONDITION]
                    fixed_parameters = map(lambda y: y.value, model.parameters[:SPECIES_PARAMETERS])
                    new_parameters = fixed_parameters + parameters          
                    x = sim.run(param_values=new_parameters)
                    temp = fitness_distance(SPECIFIC_CONDITION, x.all['MLKLobs'])
                    res.append(temp)
...
def boundary_to_platypus(B):
     res = []
     for pair in B:
          res.append(Real(pair[0], pair[1]))
     return res

def create_model_and_stuff(time):
     ...
     Observable('MLKLobs', MLKL(state='active'))

     return model


def fitness_distance(target, simulation): 
     return sqrt(sum((target-simulation)**2))

def mytrunc(mu, sigma, bounds):
     while(True):
          v = normal(mu, sigma)
          if bounds[0]<v and v<bounds[1]: return v
